...
function clientWelcomeContent(lang: string, name: string, code: string) {
  if (lang === "es") {
    return {
      ...
      useSteps: [
        "PIN: abra Configuración dentro de la app y cree su número de identificación personal (PIN). Ese PIN cancela una alerta si la activa por error. No omita este paso.",
        "Contactos de emergencia: se administran en el sitio web, no en la app. Entre a detenciondefensa.com para agregar o cambiar sus familiares de confianza; los cambios aparecen en la app automáticamente.",
        "Documentos familiares: confirme que aparecen sus documentos y toque uno para verlo.",
        "Botones inferiores: úselos para ir a Inicio, Documentos familiares y Configuración.",
        "Activar: use el control rojo de emergencia/SOS solamente cuando necesite ayuda y confirme en pantalla.",
        "Desactivar/cancelar: use el control de cancelar e ingrese su PIN. Confirme que la app diga que la alerta fue cancelada.",
  ...
  if (lang === "ht") {
    return {
      ...
      useSteps: [
        "PIN: ouvri Settings anndan app la epi kreye yon nimewo idantifikasyon pèsonèl (PIN). Se PIN sa ki anile yon alèt si ou deklanche l pa erè. Pa sote etap sa.",
        "Kontak ijans: kontak yo jere sou sit entènèt la, pa nan app la. Ale sou detenciondefensa.com pou ajoute oswa chanje moun fanmi ou fè konfyans; chanjman yo parèt nan app la otomatikman.",
        "Family Docs: verifye dokiman ou yo parèt epi peze youn pou wè li.",
        "Bouton anba yo: sèvi avèk yo pou ale nan Home, Family Docs, ak Settings.",
        "Aktive: sèvi ak kontwòl ijans/SOS wouj la sèlman lè ou bezwen èd epi konfime sou ekran an.",
        "Dezaktive/anile: sèvi ak kontwòl anile a epi antre PIN ou. Verifye app la di alèt la anile.",
  ...
  return {
    ...
    useSteps: [
      "PIN: open Settings inside the app and create your personal identification number (PIN). That PIN cancels an alert if you trigger it by accident. Do not skip this step.",
      "Emergency contacts: contacts are managed on the website, not in the app. Sign in at detenciondefensa.com to add or change your trusted family contacts; changes appear in the app automatically.",
      "Family Docs: confirm your documents appear and tap one to view it.",
      "Bottom buttons: use them to move between Home, Family Docs, and Settings.",
      "Activate: use the red emergency/SOS control only when help is needed and confirm on screen.",
      "Deactivate/cancel: use the cancel control and enter your PIN. Confirm the app says the alert was canceled.",
