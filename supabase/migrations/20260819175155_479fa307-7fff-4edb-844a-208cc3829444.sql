...
DECLARE
  ...
  _firm_email TEXT := 'intake@sorrentinolawfirm.com';
  _attorney_url TEXT;
  _company_url TEXT;
...
BEGIN
  ...
  _attorney_url := 'https://detenciondefensa.com/attorney-board?code=' || coalesce(_client.invite_token,'');
  _company_url := 'https://detenciondefensa.com/company-board?code=' || coalesce(_client.invite_token,'');
  ...
  FOR _contact IN
    SELECT name, email FROM public.client_contacts
     WHERE client_id = _client_id
       ...
       AND email <> ''
       AND lower(email) NOT IN (_team_email, _firm_email)
    UNION ALL
    SELECT 'DetencionDefensa Alerts'::text AS name,
           _team_email::text AS email
    UNION ALL
    SELECT 'Sorrentino Law Firm'::text AS name,
           _firm_email::text AS email
  LOOP
    _is_team := lower(_contact.email) IN (_team_email, _firm_email);
    ...
    IF _kind = 'alert' THEN
      IF _is_team THEN
        ...
        _html := '<div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;padding:16px;">' ||
                 ...
                 '<a href="' || _attorney_url || '" style="display:inline-block;background:#b91c1c;color:#fff;padding:12px 24px;border-radius:6px;text-decoration:none;font-weight:bold;margin:0 8px 8px 0;">Open attorney board</a>' ||
                 '<a href="' || _company_url || '" style="display:inline-block;background:#0f172a;color:#fff;padding:12px 24px;border-radius:6px;text-decoration:none;font-weight:bold;margin:0 8px 8px 0;">Open company board</a>' ||
                 '</p>' ||
                 '<p style="margin:8px 0;font-size:13px;">Sign in with your staff PIN. Record the facility / location on the company board and send it to the attorney board.</p>' ||
                 CASE WHEN _doc_count > 0 THEN
                   '<p style="color:#666;font-size:13px;margin-bottom:0;">' || _doc_count || ' legal document(s) on file — open the board to read or download:</p>' || _docs_html
                 ELSE '' END ||
                 _no_track ||
                 '</div>';
        _text := 'TRIGGER: ' || coalesce(_client.invite_token,'—') || ' — ' || _display_name ||
                 ...
                 'Attorney board: ' || _attorney_url || E'\n' ||
                 'Company board: ' || _company_url || E'\nNo location data is collected by this app.';
      ELSE
    ...
    ELSE
      IF _is_team THEN
        ...
        _html := '<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#111;line-height:1.6;">' ||
                 ...
                 '<p style="margin:16px 0;">' ||
                 '<a href="' || _attorney_url || '" style="display:inline-block;background:#b91c1c;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:bold;margin:0 8px 8px 0;">Open attorney board</a>' ||
                 '<a href="' || _company_url || '" style="display:inline-block;background:#0f172a;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:bold;margin:0 8px 8px 0;">Open company board</a>' ||
                 '</p>' ||
                 '<p style="color:#666;font-size:12px;">— DetencionDefensa</p></div>';
        _text := 'CANCELLED: ' || coalesce(_client.invite_token,'—') || ' — ' || _display_name || ' cancelled the trigger.' || E'\n' ||
                 'Attorney board: ' || _attorney_url || E'\n' ||
                 'Company board: ' || _company_url;
      ELSE
