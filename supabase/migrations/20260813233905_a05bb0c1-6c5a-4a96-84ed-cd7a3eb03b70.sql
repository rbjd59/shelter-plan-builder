-- Purge any location data already captured.
UPDATE public.client_sos_alerts
   SET lat = NULL, lng = NULL, battery_pct = NULL,
       payload = (COALESCE(payload,'{}'::jsonb)
                  - 'lat' - 'lng' - 'latitude' - 'longitude'
                  - 'last_known_location' - 'battery_pct')
 WHERE lat IS NOT NULL OR lng IS NOT NULL OR battery_pct IS NOT NULL
    OR payload ?| array['lat','lng','latitude','longitude','last_known_location','battery_pct'];

-- Rebuild the SOS email builder without any location line.
  ...
  FOR _contact IN
    SELECT name, email FROM public.client_contacts
     WHERE client_id = _client_id
       ...
       AND email <> ''
       AND lower(email) <> 'legal@theconsumerdefender.com'
    UNION ALL
    SELECT 'DetencionDefensa Legal'::text AS name,
           'legal@theconsumerdefender.com'::text AS email
  LOOP
    _is_team := lower(_contact.email) = 'legal@theconsumerdefender.com';
    ...
    IF _kind = 'alert' THEN
      IF _is_team THEN
        ...
        _html := '<div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;padding:16px;">' ||
                 '<h2 style="color:#b91c1c;margin:0 0 16px 0;">App Trigger — check the attorney board now</h2>' ||
...
-- Alert viewer no longer exposes coordinates.
