  ...
  FOR _contact IN
    SELECT name, email FROM public.client_contacts
     WHERE client_id = _client_id
       ...
       AND email <> ''
       AND lower(email) <> 'legal@detenciondefensa.com'
    UNION ALL
    SELECT 'DetencionDefensa Legal'::text AS name,
           'legal@detenciondefensa.com'::text AS email
  LOOP
    _is_team := lower(_contact.email) = 'legal@detenciondefensa.com';
    ...
    IF _kind = 'alert' THEN
      IF _is_team THEN
        ...
        _html := '<div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;padding:16px;">' ||
                 '<h2 style="color:#b91c1c;margin:0 0 16px 0;">App Trigger — check the board now</h2>' ||
        ...
        _text := 'TRIGGER: ' || coalesce(_client.invite_token,'—') || ' — ' || _display_name ||
                 ' fired an SOS. Case viewer: ' || _viewer_url;
      ELSE
    ...
    ELSE
      IF _is_team THEN
        ...
        _html := '<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#111;line-height:1.6;">' ||
                 ...
                 coalesce(_client.invite_token,'—') || ') cancelled the emergency trigger.</p>' ||
