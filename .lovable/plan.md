...
- **Email only.** No SMS / no Twilio. All notifications, install links, check-in reminders, cancel-window warnings, and trigger broadcasts go via email.
...
6. → **Email** sent to customer with **PWA install link** (unique per case, signed token).
...
- On missed deadline OR manual trigger: starts **60-min cancel window** (email warning), then if not cancelled fires `triggered=true`.
...
## Outstanding work
1. **PREREQ — Email domain** for `detenciondefensa.com` (install link, check-in reminders, cancel warnings, trigger broadcast).
2. New tables: `disclaimer_acknowledgements`, `pwa_tokens`, `dead_man_switch (case_id, interval_hours, last_checkin_at, trigger_armed_at, triggered_at)`, `pending_scrubs`.
3. Routes: `/disclaimer`, `/pwa/$token`, `/api/public/pwa/checkin`, `/api/public/pwa/trigger`, `/api/public/hooks/dms-tick` (cron, every 5 min).
4. Disclaimer short modal on landing CTA.
5. PWA manifest + install prompt + countdown UI.
6. Trigger broadcast: email to legal@ + customer + emergency contact with signed PDF links.
7. Purge cron: hard-delete after trigger+24h, after cancel+48h.
